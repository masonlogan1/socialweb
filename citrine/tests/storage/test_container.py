...
class CollectionStorageTests(TestCase):
    ...
    def test_iterkeys(self):
        ...
        for key, value in zip(keys, values):
            BTree.insert(collection, key, value)
    ...
    def test_insert(self):
        ...
        for key, value in items.items():
            self.assertEqual(BTree.get(collection, key), value)
        ...
        collection.insert('key5', 'val5')
        self.assertEqual(BTree.get(collection, 'key5'), 'val5')

        # test that attempting to write over meta will raise an exception
        with self.assertRaises(container.RestrictedItemError):
            collection.insert('meta', 'forbidden action')
    ...
    def test_update(self):
        ...
        # test that attempting to write over meta will raise an exception
        bad_update = {'meta': 'forbidden'}
        with self.assertRaises(container.RestrictedItemError):
            collection.update(bad_update)
    ...
    def test_get(self):
        ...
        # test that directly accessing the meta object is not allowed
        with self.assertRaises(container.RestrictedItemError):
            collection.get('meta')
    ...
    def test_pop(self):
        ...
        # test that attempting to pop meta will raise an exception
        with self.assertRaises(container.RestrictedItemError):
            collection.pop('meta')
    ...
    def test_setdefault(self):
        ...
        self.assertEqual(value, 'val0')

        # test that the value was updated
        self.assertEqual(collection.get('key0'), 'val0-1')

        # test that attempting to set meta will raise an exception
        with self.assertRaises(container.RestrictedItemError):
            collection.setdefault('meta', 'bad value')
